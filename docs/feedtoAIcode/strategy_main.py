...
def enrich_scored_universe(
    ...
    Also writes ``convergence_label`` so portfolio and report
    code can reference it directly from the DataFrame.
    ...
    for ticker, df in scored_universe.items():
        ...
        if "score_adjusted" in df.columns:
            ...
            # No score_adjusted column at all — create from
            # composite + delta on the last row
            comp = df.loc[last_idx, "score_composite"]
            if pd.notna(comp):
